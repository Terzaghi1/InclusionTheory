...
dip = 110;  % dip angle (degrees)
% theta = (180-dip)/180*pi;  % dip angle (rad)
...
r = 0.5*(throw - height);  % minimum y-coordinate of rectangle 1
s = 0.5*(height + throw);  % maximum y-coordinate of rectangle 1
...
y0 = 0.5*(throw - height);  % minimum y-coordinate of triangle 1
y1 = 0.5*(height + throw);  % maximum y-coordinate of triangle 1
...
% system. Additionally, this triangle is mirrored compared to the reference
% case.
[Gxx2, Gyy2, Gxy2] = strain_triangle(x - x1, y - y0, 0, x0 - x1, 0, y1 - y0, theta);
% We need to take the negative for this shape of triangle
Gxx2 = -Gxx2;
Gyy2 = -Gyy2;
Gxy2 = -Gxy2;
...
y1 = -0.5*(throw - height);  % maximum y-coordinate of triangle 2
% Note that we shift the x- and y-coordinates such that the hypotenuse of 
% the triangle goes through the origin of the coordinate system. Additionally,
% this triangle is upside down and mirrored compared to the reference case.
[Gxx3, Gyy3, Gxy3] = strain_triangle(x - x0, y - y1, 0, x1 - x0, 0, y0 - y1, theta);
% We need to take the negative for this shape of triangle
Gxx3 = -Gxx3;
Gyy3 = -Gyy3;
Gxy3 = -Gxy3;
...
s = -0.5*(throw - height);  % maximum y-coordinate of rectangle 2
...
% Here, we simply use the trick that the volumetric strain must be larger
